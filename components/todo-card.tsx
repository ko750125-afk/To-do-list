...
export function TodoCard({
  ...
  return (
    <li
      className={`
        todo-card group flex items-center justify-between gap-3 px-4 py-3.5 rounded-xl premium-card
    ...
    >
      ...
      {/* 중앙 영역: 할 일 텍스트 및 달력 기한 설정 */}
      <div className="flex-1 min-w-0 flex flex-col gap-1.5 py-0.5 justify-center">
        {isEditing ? (
          <input
            ref={inputRef}
            type="text"
            ...
            maxLength={100}
            className="w-full bg-white dark:bg-zinc-950 border border-indigo-500 rounded-lg px-3 py-1 text-sm outline-none shadow-sm shadow-indigo-100 dark:shadow-none focus:ring-2 focus:ring-indigo-100 transition-all"
          />
        ) : (
          <span
            className={`text-sm font-semibold whitespace-pre-line break-all pr-2 select-text leading-relaxed transition-all duration-200
              ${todo.done ? "line-through text-zinc-400 dark:text-zinc-600" : "text-zinc-855 dark:text-zinc-200"}
            `}
            onDoubleClick={() => setIsEditing(true)}
          >
            {todo.text.replace(/^\[고정비\]/, "[계좌이체]")}
          </span>
        )}

        {/* 달력 날짜 기한 버튼 (아래로 배치하여 텍스트의 가로공간 극대화) */}
        {!isEditing && (
          <div className="relative self-start" ref={calendarRef}>
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
              ...
              aria-label="날짜 지정"
              className={`
                flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-bold tracking-wide cursor-pointer transition-all duration-150 active:scale-95 border
                ${todo.dueDate 
